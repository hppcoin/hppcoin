...
/*
    At 15 signatures, 1/2 of the lmnode network can be owned by
...
private:
    ...
    //track lmnodes who voted with no txreq (for DOS protection)
    std::map<COutPoint, int64_t> mapLMNodeOrphanVotes; // mn outpoint - time
    ...
    int64_t GetAverageLMNodeOrphanVoteTime();
...
private:
    uint256 txHash;
    COutPoint outpoint;
    COutPoint outpointLMNode;
    std::vector<unsigned char> vchLMNodeSignature;
    // local memory only
...
public:
    ...
    CTxLockVote(const uint256& txHashIn, const COutPoint& outpointIn, const COutPoint& outpointLMNodeIn) :
        txHash(txHashIn),
        outpoint(outpointIn),
        outpointLMNode(outpointLMNodeIn),
        vchLMNodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        READWRITE(txHash);
        READWRITE(outpoint);
        READWRITE(outpointLMNode);
        READWRITE(vchLMNodeSignature);
    }
    ...
    COutPoint GetLMNodeOutpoint() const { return outpointLMNode; }
...
private:
    ...
    std::map<COutPoint, CTxLockVote> mapLMNodeVotes; // lmnode outpoint - vote
...
public:
    ...
    bool HasLMNodeVoted(const COutPoint& outpointLMNodeIn) const;
    int CountVotes() const { return mapLMNodeVotes.size(); }
...
public:
    ...
    bool HasLMNodeVoted(const COutPoint& outpointIn, const COutPoint& outpointLMNodeIn);
