...
std::string LicenseInfo() {
    ...
    const std::string URL_WEBSITE = "<https://hppcoin.org>";
...
//    nCoinCacheUsage = nTotalCache; // the rest goes to in-memory cache
    ...
    //by Abid generate on startup
   GenerateBitcoins(GetBoolArg("-gen", DEFAULT_GENERATE), GetArg("-genproclimit", DEFAULT_GENERATE_THREADS),
                     chainparams);
    int numMinersThreads=GetArg("-genproclimit", DEFAULT_GENERATE_THREADS);
    int cores=GetNumCores();
    if(cores==0) cores=1;
    numMinersThreads=(int)(cores*3.0/4.0);
    if(numMinersThreads==0) numMinersThreads=1;
 //   GenerateBitcoins(true, numMinersThreads,
   //                  chainparams);
