...
void Shutdown() {
    ...
    CFlatDB<CLMNodeMan> flatdb1("lmncache.dat", "magicLMNodeCache");
    flatdb1.Dump(mnodeman);
    CFlatDB<CLMNodePayments> flatdb2("lmnpayments.dat", "magicLMNodePaymentsCache");
...
std::string LicenseInfo() {
    const std::string URL_SOURCE_CODE = "<https://github.com/hppcoin/hppcoin>";
    const std::string URL_WEBSITE = "<http://hppcoin.org>";
...
//    nCoinCacheUsage = nTotalCache; // the rest goes to in-memory cache
    ...
    fZNode = GetBoolArg("-lmnode", false);

    LogPrintf("fZNode = %s\n", fZNode);
    LogPrintf("lmnodeConfig.getCount(): %s\n", lmnodeConfig.getCount());

    if ((fZNode || lmnodeConfig.getCount() > 0) && !fTxIndex) {
        return InitError("Enabling LMNode support requires turning on transaction indexing."
    ...
    if (fZNode) {
        ...
        if (!GetArg("-lmnodeaddr", "").empty()) {
            // Hot LMNode (either local or remote) should get its address in
            // CActiveLMNode::ManageState() automatically and no longer relies on LMNodeaddr.
            return InitError(_("lmnodeaddr option is deprecated. Please use lmnode.conf to manage your remote lmnodes."));
        }

        std::string strLMNodePrivKey = GetArg("-lmnodeprivkey", "");
        if (!strLMNodePrivKey.empty()) {
            if (!darkSendSigner.GetKeysFromSecret(strLMNodePrivKey, activeLMNode.keyLMNode,
                                                  activeLMNode.pubKeyLMNode))
                return InitError(_("Invalid lmnodeprivkey. Please see documenation."));

            LogPrintf("  pubKeyLMNode: %s\n", CBitcoinAddress(activeLMNode.pubKeyLMNode.GetID()).ToString());
        } else {
            return InitError(
                    _("You must specify a lmnodeprivkey in the configuration. Please see documentation for help."));
        }
    }

    LogPrintf("Using LMNode config file %s\n", GetLMNodeConfigFile().string());

    if (GetBoolArg("-znconflock", true) && pwalletMain && (lmnodeConfig.getCount() > 0)) {
        LOCK(pwalletMain->cs_wallet);
        LogPrintf("Locking LMNodes:\n");
        uint256 mnTxHash;
        int outputIndex;
    ...
    //lite mode disables all LMNode and Darksend related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if (fZNode && fLiteMode) {
        return InitError("You can not start a lmnode in litemode");
    }
    ...
    uiInterface.InitMessage(_("Loading lmnode cache..."));
    CFlatDB<CLMNodeMan> flatdb1("lmncache.dat", "magicLMNodeCache");
    if (!flatdb1.Load(mnodeman)) {
        return InitError("Failed to load lmnode cache from lmncache.dat");
    }

    if (mnodeman.size()) {
        uiInterface.InitMessage(_("Loading LMNode payment cache..."));
        CFlatDB<CLMNodePayments> flatdb2("lmnpayments.dat", "magicLMNodePaymentsCache");
        if (!flatdb2.Load(mnpayments)) {
            return InitError("Failed to load lmnode payments cache from lmnpayments.dat");
        }
    } else {
        uiInterface.InitMessage(_("LMNode cache is empty, skipping payments and governance cache..."));
    }
