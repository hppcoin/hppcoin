...
// A helper object for signing messages from LMNodes
...
public:
     ...
     *     1) we have an active LMNode,
     *     2) we have a valid LMNode private key,
     *     3) we signed the message successfully, and
     *     4) we verified the message successfully
     */
    bool Sign();
    /// Check if we have a valid LMNode address
    bool CheckSignature(const CPubKey& pubKeyLMNode);
    ...
    std::string ToString()
    {
        return strprintf("nDenom=%d, nTime=%lld, fReady=%s, fTried=%s, lmnode=%s",
...
{
public:
    /// Is the input associated with this public key? (and there is 1000 HPP - checking if valid lmnode)
...
private:
    ...
    // Keep track of the used LMNodes
    std::vector<CTxIn> vecLMNodesUsed;
    ...
    std::vector<CDarkSendEntry> vecEntries; // LMNode/clients entries
    ...
    /// As a client, submit part of a future mixing transaction to a LMNode to start the process
    ...
    /// Get LMNode updates about the progress of mixing
