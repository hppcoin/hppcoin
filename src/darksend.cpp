...
void CDarksendPool::ProcessMessage(CNode *pfrom, std::string &strCommand, CDataStream &vRecv) {
    ...
    if (strCommand == NetMsgType::DSACCEPT) {
        ...
        if (!fZNode) {
            LogPrintf("DSACCEPT -- not a LMNode!\n");
        ...
        CLMNode *pmn = mnodeman.Find(activeLMNode.vin);
        if (pmn == NULL) {
        ...
        CLMNode *pmn = mnodeman.Find(dsq.vin);
        if (pmn == NULL) return;

        if (!dsq.CheckSignature(pmn->pubKeyLMNode)) {
        ...
        if (dsq.fReady) {
            if (!pSubmittedToLMNode) return;
            if ((CNetAddr) pSubmittedToLMNode->addr != (CNetAddr) pmn->addr) {
                LogPrintf("DSQUEUE -- message doesn't match current LMNode: pSubmittedToLMNode=%s, addr=%s\n", pSubmittedToLMNode->addr.ToString(), pmn->addr.ToString());
                return;
            }

            if (nState == POOL_STATE_QUEUE) {
                LogPrint("privatesend", "DSQUEUE -- PrivateSend queue (%s) is ready on lmnode %s\n", dsq.ToString(), pmn->addr.ToString());
                SubmitDenominate();
            }
        } else {
            BOOST_FOREACH(CDarksendQueue
            q, vecDarksendQueue) {
                if (q.vin == dsq.vin) {
                    ...
                    LogPrint("privatesend", "DSQUEUE -- LMNode %s is sending WAY too many dsq messages\n", pmn->addr.ToString());
                    return;
                }
            }
            ...
            if (pmn->nLastDsq != 0 && nThreshold > mnodeman.nDsqCount) {
                LogPrint("privatesend", "DSQUEUE -- LMNode %s is sending too many dsq messages\n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            LogPrint("privatesend", "DSQUEUE -- new PrivateSend queue (%s) from lmnode %s\n", dsq.ToString(), pmn->addr.ToString());
            if (pSubmittedToLMNode && pSubmittedToLMNode->vin.prevout == dsq.vin.prevout) {
                dsq.fTried = true;
            }
        ...
        if (fZNode) {
            // LogPrintf("DSSTATUSUPDATE -- Can't run on a LMNode!\n");
            return;
        }

        if (!pSubmittedToLMNode) return;
        if ((CNetAddr) pSubmittedToLMNode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSSTATUSUPDATE -- message doesn't match current LMNode: pSubmittedToLMNode %s addr %s\n", pSubmittedToLMNode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (!fZNode) {
            LogPrintf("DSSIGNFINALTX -- not a LMNode!\n");
            return;
        }
        ...
        if (fZNode) {
            // LogPrintf("DSFINALTX -- Can't run on a LMNode!\n");
            return;
        }

        if (!pSubmittedToLMNode) return;
        if ((CNetAddr) pSubmittedToLMNode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSFINALTX -- message doesn't match current LMNode: pSubmittedToLMNode %s addr %s\n", pSubmittedToLMNode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (fZNode) {
            // LogPrintf("DSCOMPLETE -- Can't run on a LMNode!\n");
            return;
        }

        if (!pSubmittedToLMNode) return;
        if ((CNetAddr) pSubmittedToLMNode->addr != (CNetAddr) pfrom->addr) {
            LogPrint("privatesend", "DSCOMPLETE -- message doesn't match current LMNode: pSubmittedToLMNode=%s  addr=%s\n", pSubmittedToLMNode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
std::string CDarksendPool::GetStatus() {
    ...
    if ((pCurrentBlockIndex && pCurrentBlockIndex->nHeight - nCachedLastSuccessBlock < nMinBlockSpacing) || !lmnodeSync.IsBlockchainSynced())
    ...
    switch (nState) {
        ...
        case POOL_STATE_QUEUE:
            ...
            return strprintf(_("Submitted to lmnode, waiting in queue %s"), strSuffix);;
        case POOL_STATE_ACCEPTING_ENTRIES:
            if (nEntriesCount == 0) {
                ...
                if (nStatusMessageProgress % 70 <= 40) return strprintf(_("Submitted following entries to lmnode: %u / %d"), nEntriesCount, GetMaxPoolTransactions());
                else if (nStatusMessageProgress % 70 <= 50) strSuffix = ".";
                else if (nStatusMessageProgress % 70 <= 60) strSuffix = "..";
                else if (nStatusMessageProgress % 70 <= 70) strSuffix = "...";
                return strprintf(_("Submitted to lmnode, waiting for more entries ( %u / %d ) %s"), nEntriesCount, GetMaxPoolTransactions(), strSuffix);
            }
        case POOL_STATE_SIGNING:
...
//
// Check the mixing progress and send client updates if a LMNode
//
...
void CDarksendPool::CommitFinalTransaction() {
    if (!fZNode) return; // check and relay final tx only on lmnode
    ...
    // create and sign lmnode dstx transaction
    if (!mapDarksendBroadcastTxes.count(hashTx)) {
        CDarksendBroadcastTx dstx(finalTransaction, activeLMNode.vin, GetAdjustedTime());
        dstx.Sign();
...
//
// How does this work? Messages to LMNodes come in via NetMsgType::DSVIN, these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the LMNode
...
void CDarksendPool::CheckForCompleteQueue() {
    ...
    if (nState == POOL_STATE_QUEUE && IsSessionReady()) {
        ...
        CDarksendQueue dsq(nSessionDenom, activeLMNode.vin, GetTime(), true);
...
//
// Execute a mixing denomination via a LMNode.
// This is only ran from clients
//
bool CDarksendPool::SendDenominate(const std::vector <CTxIn> &vecTxIn, const std::vector <CTxOut> &vecTxOut) {
    if (fZNode) {
        LogPrintf("CDarksendPool::SendDenominate -- PrivateSend from a LMNode is not supported currently.\n");
        return false;
    }
    ...
    // we should already be connected to a LMNode
    if (!nSessionID) {
        LogPrintf("CDarksendPool::SendDenominate -- No LMNode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return false;
    }
...
// Incoming message from LMNode updating the progress of mixing
bool CDarksendPool::CheckPoolStateUpdate(PoolState nStateNew, int nEntriesCountNew, PoolStatusUpdate nStatusUpdate, PoolMessage nMessageID, int nSessionIDNew) {
    ...
    strAutoDenomResult = _("LMNode:") + " " + GetMessageByID(nMessageID);

    // if rejected at any state
    if (nStatusUpdate == STATUS_REJECTED) {
        LogPrintf("CDarksendPool::CheckPoolStateUpdate -- entry is rejected by LMNode\n");
        UnlockCoins();
        SetNull();
        SetState(POOL_STATE_ERROR);
...
//
// After we receive the finalized transaction from the LMNode, we must
...
bool CDarksendPool::SignFinalTransaction(const CTransaction &finalTransactionNew, CNode *pnode) {
    ...
    // push all of our signatures to the LMNode
    LogPrintf("CDarksendPool::SignFinalTransaction -- pushing sigs to the lmnode, finalMutableTransaction=%s", finalMutableTransaction.ToString());
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun) {
    ...
    if (mnodeman.size() == 0) {
        LogPrint("privatesend", "CDarksendPool::DoAutomaticDenominating -- No LMNodes detected\n");
        strAutoDenomResult = _("No LMNodes detected.");
        return false;
    }
    ...
    // If we've used 90% of the LMNode list then drop the oldest first ~30%
    int nThreshold_high = nMnCountEnabled * 0.9;
    int nThreshold_low = nThreshold_high * 0.7;
    LogPrint("privatesend", "Checking vecLMNodesUsed: size: %d, threshold: %d\n", (int) vecLMNodesUsed.size(), nThreshold_high);

    if ((int) vecLMNodesUsed.size() > nThreshold_high) {
        vecLMNodesUsed.erase(vecLMNodesUsed.begin(), vecLMNodesUsed.begin() + vecLMNodesUsed.size() - nThreshold_low);
        LogPrint("privatesend", "  vecLMNodesUsed: new size: %d, threshold: %d\n", (int) vecLMNodesUsed.size(), nThreshold_high);
    }
    ...
    if (nLiquidityProvider || fUseQueue) {
        ...
        BOOST_FOREACH(CDarksendQueue & dsq, vecDarksendQueue)
        {
            ...
            CLMNode *pmn = mnodeman.Find(dsq.vin);
            if (pmn == NULL) {
                LogPrintf("CDarksendPool::DoAutomaticDenominating -- dsq lmnode is not in lmnode list, lmnode=%s\n", dsq.vin.prevout.ToStringShort());
                continue;
            }
            ...
            vecLMNodesUsed.push_back(dsq.vin);
            ...
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt to connect to lmnode from queue, addr=%s\n", pmn->addr.ToString());
            // connect to LMNode and submit the queue request
            CNode *pnode = (pnodeFound && pnodeFound->fLMNode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
            if (pnode) {
                pSubmittedToLMNode = pmn;
                ...
                strAutoDenomResult = _("Error connecting to LMNode.");
                continue;
            }
        }
    }
    ...
    while (nTries < 10) {
        CLMNode *pmn = mnodeman.FindRandomNotInVec(vecLMNodesUsed, MIN_PRIVATESEND_PEER_PROTO_VERSION);
        if (pmn == NULL) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Can't find random lmnode!\n");
            strAutoDenomResult = _("Can't find random LMNode.");
            return false;
        }
        vecLMNodesUsed.push_back(pmn->vin);

        if (pmn->nLastDsq != 0 && pmn->nLastDsq + nMnCountEnabled / 5 > mnodeman.nDsqCount) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Too early to mix on this lmnode!"
                              " lmnode=%s  addr=%s  nLastDsq=%d  CountEnabled/5=%d  nDsqCount=%d\n",
        ...
        LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt %d connection to LMNode %s\n", nTries, pmn->addr.ToString());
        CNode *pnode = (pnodeFound && pnodeFound->fLMNode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
        if (pnode) {
    ...
    strAutoDenomResult = _("No compatible LMNode found.");
    return false;
}
...
bool CDarksendPool::CreateNewSession(int nDenom, CTransaction txCollateral, PoolMessage &nMessageIDRet) {
    ...
    if (!fUnitTest) {
        ...
        CDarksendQueue dsq(nDenom, activeLMNode.vin, GetTime(), false);
...
std::string CDarksendPool::GetMessageByID(PoolMessage nMessageID) {
    switch (nMessageID) {
        ...
        case ERR_MN_LIST:
            return _("Not in the LMNode list.");
        case ERR_MODE:
            return _("Incompatible mode.");
        case ERR_NON_STANDARD_PUBKEY:
            return _("Non-standard public key detected.");
        case ERR_NOT_A_MN:
            return _("This is not a LMNode.");
        case ERR_QUEUE_FULL:
            return _("LMNode queue is full.");
        case ERR_RECENT:
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn &txin, const CPubKey &pubkey) {
    ...
    if (GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        ...
        if (out.nValue == LMNODE_COIN_REQUIRED * COIN && out.scriptPubKey == payee) return true;
    }
...
bool CDarksendQueue::Sign() {
    ...
    return CheckSignature(activeLMNode.pubKeyLMNode);
}

bool CDarksendQueue::CheckSignature(const CPubKey &pubKeyLMNode) {
    std::string strMessage = vin.ToString() + boost::lexical_cast<std::string>(nDenom) + boost::lexical_cast<std::string>(nTime) + boost::lexical_cast<std::string>(fReady);
    std::string strError = "";

    if (!darkSendSigner.VerifyMessage(pubKeyLMNode, vchSig, strMessage, strError)) {
        LogPrintf("CDarksendQueue::CheckSignature -- Got bad LMNode queue signature: %s; error: %s\n", ToString(), strError);
        return false;
    }
...
bool CDarksendBroadcastTx::CheckSignature(const CPubKey &pubKeyLMNode) {
    std::string strMessage = tx.GetHash().ToString() + boost::lexical_cast<std::string>(sigTime);
    std::string strError = "";
...
void CDarksendPool::RelayIn(const CDarkSendEntry &entry) {
    ...
    CNode *pnode = FindNode(pSubmittedToLMNode->addr);
    if (pnode != NULL) {
...
void CDarksendPool::SetState(PoolState nStateNew) {
    if (fZNode && (nStateNew == POOL_STATE_ERROR || nStateNew == POOL_STATE_SUCCESS)) {
        LogPrint("privatesend", "CDarksendPool::SetState -- Can't set state to ERROR or SUCCESS as a LMNode. \n");
        return;
    }
...
void CDarksendPool::UpdatedBlockTip(const CBlockIndex *pindex) {
    ...
    if (!fLiteMode && lmnodeSync.IsLMNodeListSynced()) {
        NewBlock();
    }
}
...
void ThreadCheckDarkSendPool() {
    ...
    while (true) {
        ...
        lmnodeSync.ProcessTick();

        if (lmnodeSync.IsBlockchainSynced() && !ShutdownRequested()) {

            nTick++;

            // make sure to check all lmnodes first
            mnodeman.Check();

            // check if we should activate or ping every few minutes,
            // slightly postpone first run to give net thread a chance to connect to some peers
            if (nTick % LMNODE_MIN_MNP_SECONDS == 15)
                activeLMNode.ManageState();

            if (nTick % 60 == 0) {
                mnodeman.ProcessLMNodeConnections();
