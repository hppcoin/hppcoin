...
CBlockTemplate* BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    // To founders and investors
     if ((nHeight > 0) && (nHeight < 262500)) {
        ...
        coinbaseTx.vout.push_back(CTxOut(1 * COIN, CScript(FOUNDER_8_SCRIPT.begin(), FOUNDER_8_SCRIPT.end())));
        } else if (nHeight >= Params().GetConsensus().nLMNodePaymentsStartBlock) {
            // Take some reward away from us
            coinbaseTx.vout[0].nValue = -8 * COIN;
            ...
            coinbaseTx.vout.push_back(CTxOut(1 * COIN, CScript(FOUNDER_5_SCRIPT.begin(), FOUNDER_5_SCRIPT.end())));
            coinbaseTx.vout.push_back(CTxOut(1 * COIN, CScript(FOUNDER_6_SCRIPT.begin(), FOUNDER_6_SCRIPT.end())));
            coinbaseTx.vout.push_back(CTxOut(1.5 * COIN, CScript(FOUNDER_7_SCRIPT.begin(), FOUNDER_7_SCRIPT.end())));
            coinbaseTx.vout.push_back(CTxOut(0.5 * COIN, CScript(FOUNDER_8_SCRIPT.begin(), FOUNDER_8_SCRIPT.end())));
...
//            }
        ...
        // Update coinbase transaction with additional info about lmnode and governance payments,
        // get some info back to pass to getblocktemplate
        if (nHeight >= chainparams.GetConsensus().nLMNodePaymentsStartBlock) {
            CAmount lmnodePayment = GetLMNodePayment(nHeight, blockReward);
            coinbaseTx.vout[0].nValue -= lmnodePayment;
            FillBlockPayments(coinbaseTx, nHeight, lmnodePayment, pblock->txoutLMNode, pblock->voutSuperblock);
        }
