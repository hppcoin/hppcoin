...
CBlockTemplate* BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    if(!pblocktemplate.get())
        {
        LogPrintf("pblocktemplate(new CBlockTemplate()) retrned NULL\n");
        return NULL;
    ...
    if ((nHeight > 0) && (nHeight < 262500)) {
        ...
        txNew.vout.push_back(CTxOut(1 * COIN, CScript(FOUNDER_1_SCRIPT.begin(), FOUNDER_1_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(1 * COIN, CScript(FOUNDER_2_SCRIPT.begin(), FOUNDER_2_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(1 * COIN, CScript(FOUNDER_3_SCRIPT.begin(), FOUNDER_3_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(1 * COIN, CScript(FOUNDER_4_SCRIPT.begin(), FOUNDER_4_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(1 * COIN, CScript(FOUNDER_5_SCRIPT.begin(), FOUNDER_5_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(1 * COIN, CScript(FOUNDER_6_SCRIPT.begin(), FOUNDER_6_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(1 * COIN, CScript(FOUNDER_7_SCRIPT.begin(), FOUNDER_7_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(1 * COIN, CScript(FOUNDER_8_SCRIPT.begin(), FOUNDER_8_SCRIPT.end())));
...
CBlockTemplate* BlockAssembler::CreateNewBlock_(const CScript& scriptPubKeyIn)
{
   ...
   {    
 LogPrintf("pblocktemplate.get() NULL \n");
    return NULL;
    ...
    // To founders and investors
    if ((nHeight > 0) && (nHeight + 1 < 262500)) {
        // Take some reward for developers team and investors
        coinbaseTx.vout[0].nValue = -8 * COIN;
        ...
        coinbaseTx.vout.push_back(CTxOut(1 * COIN, CScript(FOUNDER_6_SCRIPT.begin(), FOUNDER_6_SCRIPT.end())));
        coinbaseTx.vout.push_back(CTxOut(1 * COIN, CScript(FOUNDER_7_SCRIPT.begin(), FOUNDER_7_SCRIPT.end())));
        coinbaseTx.vout.push_back(CTxOut(1 * COIN, CScript(FOUNDER_8_SCRIPT.begin(), FOUNDER_8_SCRIPT.end())));
    }
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock, true, NULL, false))
        return error("HppcoinMiner: ProcessNewBlock, block not accepted");

    return true;
}

void static HppcoinMiner(const CChainParams &chainparams) {
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hppcoin-miner");
    ...
    try {
        ...
        if (!coinbaseScript || coinbaseScript->reserveScript.empty()) {
            LogPrintf("HppcoinMiner stop here coinbaseScript=%s, coinbaseScript->reserveScript.empty()=%s\n", coinbaseScript, coinbaseScript->reserveScript.empty());
        ...
        while (true) {
          ...
          // commented by Abid
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("Error in HppcoinMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running HppcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
                      ::GetSerializeSize(*pblock, SER_NETWORK, PROTOCOL_VERSION));
            ...
            while (true) {
                ...
                while (true) {
                    if ((!fTestNet && pindexPrev->nHeight  >= 0)) {
                        lyra2h_hash(BEGIN(pblock->nVersion), BEGIN(thash));
                    } 
...
//                        CheckWork(pblock, *pwallet, reservekey);
        ...
        LogPrintf("HppcoinMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e) {
        LogPrintf("HppcoinMiner runtime error: %s\n", e.what());
        return;
    }
}
...
void GenerateBitcoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HppcoinMiner, boost::cref(chainparams)));
}
