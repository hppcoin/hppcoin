...
// step 2) Top COutPointLock::SIGNATURES_TOTAL lmnodes per each spent outpoint push "txvote" message
...
//    if(!sporkManager.IsSporkActive(SPORK_2_INSTANTSEND_ENABLED)) return;
    ...
    // Ignore any InstantSend messages until lmnode list is synced
...
bool CInstantSend::ProcessTxLockRequest(const CTxLockRequest& txLockRequest)
{
    ...
    // LMNodes will sometimes propagate votes before the transaction is known to the client.
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        int n = mnodeman.GetLMNodeRank(activeLMNode.vin, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

        if(n == -1) {
            LogPrint("instantsend", "CInstantSend::Vote -- Unknown LMNode %s\n", activeLMNode.vin.prevout.ToStringShort());
            ++itOutpointLock;
            continue;
        }
        ...
        if(n > nSignaturesTotal) {
            LogPrint("instantsend", "CInstantSend::Vote -- LMNode not in the top %d (%d)\n", nSignaturesTotal, n);
            ++itOutpointLock;
            continue;
        }
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            BOOST_FOREACH(const uint256& hash, itVoted->second) {
                ...
                if(it2->second.HasLMNodeVoted(itOutpointLock->first, activeLMNode.vin.prevout)) {
        ...
        CTxLockVote vote(txHash, itOutpointLock->first, activeLMNode.vin.prevout);
...
bool CInstantSend::ProcessTxLockVote(CNode* pfrom, CTxLockVote& vote)
{
    ...
    if(it == mapTxLockCandidates.end()) {
        if(!mapTxLockVotesOrphan.count(vote.GetHash())) {
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  lmnode=%s new\n",
                    txHash.ToString(), vote.GetLMNodeOutpoint().ToStringShort());
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  lmnode=%s seen\n",
                    txHash.ToString(), vote.GetLMNodeOutpoint().ToStringShort());
        }
        ...
        int nLMNodeOrphanExpireTime = GetTime() + 60*10; // keep time data for 10 minutes
        if(!mapLMNodeOrphanVotes.count(vote.GetLMNodeOutpoint())) {
            mapLMNodeOrphanVotes[vote.GetLMNodeOutpoint()] = nLMNodeOrphanExpireTime;
        } else {
            int64_t nPrevOrphanVote = mapLMNodeOrphanVotes[vote.GetLMNodeOutpoint()];
            if(nPrevOrphanVote > GetTime() && nPrevOrphanVote > GetAverageLMNodeOrphanVoteTime()) {
                LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- lmnode is spamming orphan Transaction Lock Votes: txid=%s  lmnode=%s\n",
                        txHash.ToString(), vote.GetLMNodeOutpoint().ToStringShort());
    ...
    if(it1 != mapVotedOutpoints.end()) {
        BOOST_FOREACH(const uint256& hash, it1->second) {
            if(hash != txHash) {
                ...
                std::map<uint256, CTxLockCandidate>::iterator it2 = mapTxLockCandidates.find(hash);
                if(it2->second.HasLMNodeVoted(vote.GetOutpoint(), vote.GetLMNodeOutpoint())) {
                    // yes, it did, refuse to accept a vote to include the same outpoint in another tx
                    // from the same lmnode.
                    // TODO: apply pose ban score to this lmnode?
                    ...
                    // pose ban score too.
                    LogPrintf("CInstantSend::ProcessTxLockVote -- lmnode sent conflicting votes! %s\n", vote.GetLMNodeOutpoint().ToStringShort());
                    return false;
                }
            }
        }
        // we have votes by other lmnodes only (so far), let's continue and see who will win
...
int64_t CInstantSend::GetAverageLMNodeOrphanVoteTime()
{
    LOCK(cs_instantsend);
    // NOTE: should never actually call this function when mapLMNodeOrphanVotes is empty
    if(mapLMNodeOrphanVotes.empty()) return 0;

    std::map<COutPoint, int64_t>::iterator it = mapLMNodeOrphanVotes.begin();
    int64_t total = 0;

    while(it != mapLMNodeOrphanVotes.end()) {
        total+= it->second;
        ++it;
    }

    return total / mapLMNodeOrphanVotes.size();
}
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsExpired(pCurrentBlockIndex->nHeight)) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired vote: txid=%s  lmnode=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetLMNodeOutpoint().ToStringShort());
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(GetTime() - itOrphanVote->second.GetTimeCreated() > ORPHAN_VOTE_SECONDS) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan vote: txid=%s  lmnode=%s\n",
                    itOrphanVote->second.GetTxHash().ToString(), itOrphanVote->second.GetLMNodeOutpoint().ToStringShort());
    ...
    // remove expired lmnode orphan votes (DOS protection)
    std::map<COutPoint, int64_t>::iterator itLMNodeOrphan = mapLMNodeOrphanVotes.begin();
    while(itLMNodeOrphan != mapLMNodeOrphanVotes.end()) {
        if(itLMNodeOrphan->second < GetTime()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan lmnode vote: lmnode=%s\n",
                    itLMNodeOrphan->first.ToStringShort());
            mapLMNodeOrphanVotes.erase(itLMNodeOrphan++);
        } else {
            ++itLMNodeOrphan;
        }
    }
...
{
    if(!mnodeman.Has(CTxIn(outpointLMNode))) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Unknown lmnode %s\n", outpointLMNode.ToStringShort());
        mnodeman.AskForMN(pnode, CTxIn(outpointLMNode));
        return false;
    }
    ...
    int n = mnodeman.GetLMNodeRank(CTxIn(outpointLMNode), nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

    if(n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantsend", "CTxLockVote::IsValid -- Outdated lmnode %s\n", outpointLMNode.ToStringShort());
        return false;
    }
    LogPrint("instantsend", "CTxLockVote::IsValid -- LMNode %s, rank=%d\n", outpointLMNode.ToStringShort(), n);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(n > nSignaturesTotal) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- LMNode %s is not in the top %d (%d), vote hash=%s\n",
                outpointLMNode.ToStringShort(), nSignaturesTotal, n, GetHash().ToString());
        return false;
    }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    lmnode_info_t infoMn = mnodeman.GetLMNodeInfo(CTxIn(outpointLMNode));

    if(!infoMn.fInfoValid) {
        LogPrintf("CTxLockVote::CheckSignature -- Unknown LMNode: lmnode=%s\n", outpointLMNode.ToString());
        return false;
    }

    if(!darkSendSigner.VerifyMessage(infoMn.pubKeyLMNode, vchLMNodeSignature, strMessage, strError)) {
...
bool CTxLockVote::Sign()
{
    ...
    if(!darkSendSigner.SignMessage(strMessage, vchLMNodeSignature, activeLMNode.keyLMNode)) {
        LogPrintf("CTxLockVote::Sign -- SignMessage() failed\n");
        return false;
    }

    if(!darkSendSigner.VerifyMessage(activeLMNode.pubKeyLMNode, vchLMNodeSignature, strMessage, strError)) {
...
{
    if(mapLMNodeVotes.count(vote.GetLMNodeOutpoint()))
        return false;
    mapLMNodeVotes.insert(std::make_pair(vote.GetLMNodeOutpoint(), vote));
    return true;
}
...
std::vector<CTxLockVote> COutPointLock::GetVotes() const
{
    ...
    std::map<COutPoint, CTxLockVote>::const_iterator itVote = mapLMNodeVotes.begin();
    while(itVote != mapLMNodeVotes.end()) {
...
bool COutPointLock::HasLMNodeVoted(const COutPoint& outpointLMNodeIn) const
{
    return mapLMNodeVotes.count(outpointLMNodeIn);
}
...
bool CTxLockCandidate::HasLMNodeVoted(const COutPoint& outpointIn, const COutPoint& outpointLMNodeIn)
{
    std::map<COutPoint, COutPointLock>::iterator it = mapOutPointLocks.find(outpointIn);
    return it !=mapOutPointLocks.end() && it->second.HasLMNodeVoted(outpointLMNodeIn);
}
