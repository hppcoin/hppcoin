...
#if defined(NDEBUG)
# error "Hppcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Hppcoin Signed Message:\n";
...
bool CheckSpendHppcoinTransaction(const CTransaction &tx, CZerocoinEntry pubCoinTx, list <CZerocoinEntry> listPubCoin,
    ...
    LogPrintf("CheckSpendHppcoinTransaction denomination=%d nHeight=%d\n", targetDenomination, nHeight);
    BOOST_FOREACH(
    const CTxIn &txin, tx.vin)
    {
        if (txin.scriptSig.IsZerocoinSpend()) {
            ...
            if (!passVerify) {
                int countPubcoin = 0;
//                LogPrint("CheckSpendHppcoinTransaction", "Check reverse\n");
...
//                              pubCoinItem.denomination, pubCoinItem.id, pubcoinId, pubCoinItem.nHeight);
                    if (pubCoinItem.denomination == targetDenomination &&
                        (pubCoinItem.id >= 0 && (uint32_t) pubCoinItem.id == pubcoinId) &&
                        pubCoinItem.nHeight != -1) {
                        LogPrint("CheckSpendHppcoinTransaction",
                        ...
                        if (item.coinSerial == serialNumber
                            ...
                            const CZerocoinEntry &pubCoinItem, listPubCoin) {
                                if (pubCoinItem.value == item.pubCoin) {
                                    ...
                                    // Update UI wallet
                                    // LogPrint("CheckSpendHppcoinTransaction", "pubcoin=%s, isUsed=Used\n", pubCoinItem.value.GetHex());
...
//    LogPrintf("transaction = %s\n", tx.ToString());
    ...
    if (tx.IsCoinBase()) {
        ...
        CScript FOUNDER_8_SCRIPT;
            if (nHeight < Params().GetConsensus().nLMNodePaymentsStartBlock) {
               if (!fTestNet && (GetAdjustedTime() > nStartRewardTime)) {
            FOUNDER_1_SCRIPT = GetScriptForDestination(CBitcoinAddress("hWZ12AMH29Majik1ZqWQ7LX76QyUtNHzgx").Get());
            FOUNDER_2_SCRIPT = GetScriptForDestination(CBitcoinAddress("hbQxRVWep3pt54tPPW97SeZdNiX8TtYVyZ").Get());
            FOUNDER_3_SCRIPT = GetScriptForDestination(CBitcoinAddress("hVGCQSWMFgsi3WD4ncVnkGscANhTnjsvk9").Get());
            FOUNDER_4_SCRIPT = GetScriptForDestination(CBitcoinAddress("hHmb8RyJ7arJRAqfgMvUxrmjzMKa41F3AJ").Get());
            FOUNDER_5_SCRIPT = GetScriptForDestination(CBitcoinAddress("hZU41qT4CMTPn1JangDkjA4WRWnq2s4Juj").Get());
            FOUNDER_6_SCRIPT = GetScriptForDestination(CBitcoinAddress("hVwW6dNSDggWjJfejGYAc9ZZFeBmnT7qsC").Get());
            FOUNDER_7_SCRIPT = GetScriptForDestination(CBitcoinAddress("hHA3VU7rdVGcEkQM7BFa5dg7W3K3Go2b2v").Get());
            FOUNDER_8_SCRIPT = GetScriptForDestination(CBitcoinAddress("hKB454Y97LW7RgUYLBTSF99D9x3VggrK9J").Get());
            ...
            FOUNDER_1_SCRIPT = GetScriptForDestination(CBitcoinAddress("HBqQBUEMX4vaQaGLfvoxYjC11J38JzNgPD").Get());
            FOUNDER_2_SCRIPT = GetScriptForDestination(CBitcoinAddress("HUXbm2jNZQknEHfvCRmHARq5hujjkZru16").Get());
            FOUNDER_3_SCRIPT = GetScriptForDestination(CBitcoinAddress("HPySwwDQjufYErEZwqzGDAti4x8R4fP49G").Get());
            FOUNDER_4_SCRIPT = GetScriptForDestination(CBitcoinAddress("HCxv35iuAN3h4GYhGCbmNsFCV2J1h9KTVx").Get());
            FOUNDER_5_SCRIPT = GetScriptForDestination(CBitcoinAddress("HFTh9vtVM4JLDyChB6y1yMS83YqV4ywN1d").Get());
            FOUNDER_6_SCRIPT = GetScriptForDestination(CBitcoinAddress("HD2yGsFtVMBe3gkX3HW4uWdXdwa6SS2DZP").Get());
            FOUNDER_7_SCRIPT = GetScriptForDestination(CBitcoinAddress("H9aUmPeqcEDLnjGZhHcjNUpa1z9gRhAwxv").Get());
            FOUNDER_8_SCRIPT = GetScriptForDestination(CBitcoinAddress("HGLt4mteYoaBSYFvGaKZKVmgYFTP3KZERF").Get());
                ...
                if (output.scriptPubKey == FOUNDER_6_SCRIPT && output.nValue == (int64_t)(1 * COIN)) {
                    found_6 = true;
                }
                if (output.scriptPubKey == FOUNDER_7_SCRIPT && output.nValue == (int64_t)(1 * COIN)) {
                    found_7 = true;
                }
                if (output.scriptPubKey == FOUNDER_8_SCRIPT && output.nValue == (int64_t)(1 * COIN)) {
                    found_8 = true;
                }
                ...
                if (!fTestNet && GetAdjustedTime() > nStartRewardTime) {
            FOUNDER_1_SCRIPT = GetScriptForDestination(CBitcoinAddress("hKB454Y97LW7RgUYLBTSF99D9x3VggrK9J").Get());
            FOUNDER_2_SCRIPT = GetScriptForDestination(CBitcoinAddress("hbQxRVWep3pt54tPPW97SeZdNiX8TtYVyZ").Get());
            FOUNDER_3_SCRIPT = GetScriptForDestination(CBitcoinAddress("hVGCQSWMFgsi3WD4ncVnkGscANhTnjsvk9").Get());
            FOUNDER_4_SCRIPT = GetScriptForDestination(CBitcoinAddress("hHmb8RyJ7arJRAqfgMvUxrmjzMKa41F3AJ").Get());
            FOUNDER_5_SCRIPT = GetScriptForDestination(CBitcoinAddress("hZU41qT4CMTPn1JangDkjA4WRWnq2s4Juj").Get());
            FOUNDER_6_SCRIPT = GetScriptForDestination(CBitcoinAddress("hVwW6dNSDggWjJfejGYAc9ZZFeBmnT7qsC").Get());
            FOUNDER_7_SCRIPT = GetScriptForDestination(CBitcoinAddress("hHA3VU7rdVGcEkQM7BFa5dg7W3K3Go2b2v").Get());
            FOUNDER_8_SCRIPT = GetScriptForDestination(CBitcoinAddress("hNKisatBSWWxtz3swk7asfw8eG8oCF71x7").Get());
                } else if (!fTestNet && GetAdjustedTime() <= nStartRewardTime) {
                    return state.DoS(100, false, REJECT_TRANSACTION_TOO_EARLY,
                                     "CTransaction::CheckTransaction() : transaction is too early");
                } else {
            ...
            FOUNDER_8_SCRIPT = GetScriptForDestination(CBitcoinAddress("HGLt4mteYoaBSYFvGaKZKVmgYFTP3KZERF").Get());
                ...
                CAmount lmnodePayment = GetLMNodePayment(nHeight);
                BOOST_FOREACH(const CTxOut &output, tx.vout) {
                     if (output.scriptPubKey == FOUNDER_1_SCRIPT && output.nValue == (int64_t)(1 * COIN)) {
                    found_1 = true;
                }
                ...
                if (output.scriptPubKey == FOUNDER_7_SCRIPT && output.nValue == (int64_t)(1.5 * COIN)) {
                    found_7 = true;
                }
                if (output.scriptPubKey == FOUNDER_8_SCRIPT && output.nValue == (int64_t)(0.5 * COIN)) {
                    found_8 = true;
                }
                    if (lmnodePayment == output.nValue) {
            ...
            if (total_payment_tx > 1) {
                return state.DoS(100, false, REJECT_INVALID_LMNODE_PAYMENT,
                                 "CTransaction::CheckTransaction() : invalid lmnode payment");
            }
        }
    } else {
        ...
        BOOST_FOREACH(const CTxOut &txout, tx.vout) {
            if (!txout.scriptPubKey.empty() && txout.scriptPubKey.IsZerocoinMint()) {
                LogPrintf("CheckMintHppcoinTransaction txHash = %s, isVerifyDB = %d\n", txout.GetHash().ToString(),
                          isVerifyDB);
                LogPrintf("nValue = %d\n", txout.nValue);
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            BOOST_FOREACH(const CTxOut &txout, tx.vout)
            {
                ...
                if (!isVerifyDB) {
                    ...
                    if (txout.nValue == libzerocoin::ZQ_LOVELACE * COIN) {
                        // Check vIn
                        if (!CheckSpendHppcoinTransaction(tx, pubCoinTx, listPubCoin, libzerocoin::ZQ_LOVELACE, state,
                                                        hashTx, isVerifyDB, nHeight, isCheckWallet)) {
                            return state.DoS(100,
                                             error("CTransaction::CheckTransaction() : COIN SPEND TX IN ZQ_LOVELACE DID NOT VERIFY!"));
                        };
                    } else if (txout.nValue == libzerocoin::ZQ_GOLDWASSER * COIN) {
                        if (!CheckSpendHppcoinTransaction(tx, pubCoinTx, listPubCoin, libzerocoin::ZQ_GOLDWASSER, state,
                                                        hashTx, isVerifyDB, nHeight, isCheckWallet)) {
                            return state.DoS(100,
                                             error("CTransaction::CheckTransaction() : COIN SPEND TX IN ZQ_GOLDWASSER DID NOT VERIFY!"));
                        };
                    } else if (txout.nValue == libzerocoin::ZQ_RACKOFF * COIN) {
                        if (!CheckSpendHppcoinTransaction(tx, pubCoinTx, listPubCoin, libzerocoin::ZQ_RACKOFF, state,
                                                        hashTx, isVerifyDB, nHeight, isCheckWallet)) {
                            return state.DoS(100,
                                             error("CTransaction::CheckTransaction() : COIN SPEND TX IN ZQ_RACKOFF DID NOT VERIFY!"));
                        };
                    } else if (txout.nValue == libzerocoin::ZQ_PEDERSEN * COIN) {
                        if (!CheckSpendHppcoinTransaction(tx, pubCoinTx, listPubCoin, libzerocoin::ZQ_PEDERSEN, state,
                                                        hashTx, isVerifyDB, nHeight, isCheckWallet)) {
                            return state.DoS(100,
                                             error("CTransaction::CheckTransaction() : COIN SPEND TX IN ZQ_PEDERSEN DID NOT VERIFY!"));
                        };
                    } else if (txout.nValue == libzerocoin::ZQ_WILLIAMSON * COIN) {
                        if (!CheckSpendHppcoinTransaction(tx, pubCoinTx, listPubCoin, libzerocoin::ZQ_WILLIAMSON, state,
                                                        hashTx, isVerifyDB, nHeight, isCheckWallet)) {
                            return state.DoS(100,
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params &consensusParams, int nTime) {
    ...
    // Subsidy is cut in half every 262500 blocks which will occur approximately every 2 years.
    nSubsidy >>= halvings;
    return nSubsidy;
}
...
//                                      !(pindexBIP34height->GetBlockHash() == chainparams.GetConsensus().BIP34Hash));
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (txIds.count(txHash) > 0 && (fTestNet || pindex->nHeight >= HF_LMNODE_HEIGHT))
    ...
    // LMNODE : MODIFIED TO CHECK LMNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        ...
        return state.DoS(0, error("ConnectBlock(): couldn't find lmnode or superblock payments"),
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams &chainParams) {
    ...
    lmnodeSync.UpdatedBlockTip(chainActive.Tip());
...
CAmount GetLMNodePayment(int nHeight, CAmount blockValue) {
    CAmount ret = 20 * COIN; //20HPP
if(nHeight>262500 && nHeight<262500*2+1)
    ret = 10 * COIN; //10HPP
    
if(nHeight>262500*2 && nHeight<262500*3+1)
    ret = 5 * COIN; //5HPP

if(nHeight>262500*3 && nHeight<262500*4+1)
    ret = 2.5 * COIN; //2.5HPP

if(nHeight>262500*4 && nHeight<262500*5+1)
    ret = 1.25 * COIN; //1.25HPP

if(nHeight>262500*5 && nHeight<262500*6+1)
    ret = 0.625 * COIN; //1.25HPP
 ...
 * corresponding to pindexNew, to bypass loading it again from disk.
 */
bool static ReArrangeHppcoinMint(CValidationState &state, const CChainParams &chainparams, CBlockIndex *pindexNew,
                               const CBlock *pblock) {
//    LogPrintf("[ReArrangeHppcoinMint]\n");
    CBlock block;
    if (!pblock) {
        if (!ReadBlockFromDisk(block, pindexNew, chainparams.GetConsensus()))
            return AbortNode(state, "Failed to read block");
...
//    LogPrintf("[ActivateBestChainStep] fContinue=%d, nHeight=%d, pindexMostWork->nHeight=%d\n", fContinue, nHeight, pindexMostWork->nHeight);
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        ...
        {
            ReArrangeHppcoinMint(state, chainparams, pindexConnect, pindexConnect == pindexMostWork ? pblock : NULL);
        }
    }
...
bool CheckBlock(const CBlock &block, CValidationState &state, const Consensus::Params &consensusParams, bool fCheckPOW,
    ...
    try {
        ...
        if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
            ...
            BOOST_FOREACH(const CTransaction& tx, block.vtx) {
                ...
                BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                    ...
                    if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                        ...
                        LOCK(cs_main);
                        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                        return state.DoS(0, error("CheckBlock(HPP): transaction %s conflicts with transaction lock %s",
            ...
            LogPrintf("CheckBlock(HPP): spork is off, skipping transaction locking checks\n");
        }
...
bool ProcessNewBlock(CValidationState &state, const CChainParams &chainparams, CNode *pfrom, const CBlock *pblock,
    ...
    lmnodeSync.IsBlockchainSynced(true);
...
bool static AlreadyHave(const CInv &inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    */
        ...
        case MSG_LMNODE_PAYMENT_VOTE:
            return mnpayments.mapLMNodePaymentVotes.count(inv.hash);

        case MSG_LMNODE_PAYMENT_BLOCK:
        {
            BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
            return mi != mapBlockIndex.end() && mnpayments.mapLMNodeBlocks.find(mi->second->nHeight) != mnpayments.mapLMNodeBlocks.end();
        }

        case MSG_LMNODE_ANNOUNCE:
            return mnodeman.mapSeenLMNodeBroadcast.count(inv.hash) && !mnodeman.IsMnbRecoveryRequested(inv.hash);

        case MSG_LMNODE_PING:
            return mnodeman.mapSeenLMNodePing.count(inv.hash);

        case MSG_DSTX:
            return mapDarksendBroadcastTxes.count(inv.hash);

        case MSG_LMNODE_VERIFY:
            return mnodeman.mapSeenLMNodeVerification.count(inv.hash);
    }
...
//                LogPrintf("inv.type()=%s, inv.GetCommand=%s\n", inv.type, inv.GetCommand());
                ...
                if (!pushed && inv.type == MSG_LMNODE_PAYMENT_VOTE) {
                    if(mnpayments.HasVerifiedPaymentVote(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnpayments.mapLMNodePaymentVotes[inv.hash];
                        pfrom->PushMessage(NetMsgType::LMNODEPAYMENTVOTE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_LMNODE_PAYMENT_BLOCK) {
                    BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
                    LOCK(cs_mapLMNodeBlocks);
                    if (mi != mapBlockIndex.end() && mnpayments.mapLMNodeBlocks.count(mi->second->nHeight)) {
                        BOOST_FOREACH(CLMNodePayee& payee, mnpayments.mapLMNodeBlocks[mi->second->nHeight].vecPayees) {
                            std::vector<uint256> vecVoteHashes = payee.GetVoteHashes();
                            BOOST_FOREACH(uint256& hash, vecVoteHashes) {
                                if(mnpayments.HasVerifiedPaymentVote(hash)) {
                                    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                                    ss.reserve(1000);
                                    ss << mnpayments.mapLMNodePaymentVotes[hash];
                                    pfrom->PushMessage(NetMsgType::LMNODEPAYMENTVOTE, ss);
                                }
                            }
                        }
                ...
                if (!pushed && inv.type == MSG_LMNODE_ANNOUNCE) {
                    if(mnodeman.mapSeenLMNodeBroadcast.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenLMNodeBroadcast[inv.hash].second;
                        pfrom->PushMessage(NetMsgType::MNANNOUNCE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_LMNODE_PING) {
                    if(mnodeman.mapSeenLMNodePing.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenLMNodePing[inv.hash];
                ...
                if (!pushed && inv.type == MSG_LMNODE_VERIFY) {
                    if(mnodeman.mapSeenLMNodeVerification.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenLMNodeVerification[inv.hash];
...
bool static ProcessMessage(CNode *pfrom, string strCommand, CDataStream &vRecv, int64_t nTimeReceived,
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        int minPeerVersion = (nHeight + 1 < HF_LMNODE_HEIGHT) ? MIN_PEER_PROTO_VERSION : MIN_PEER_PROTO_VERSION_AFTER_LMNODE_PAYMENT_HF;
...
//        vRecv >> tx;
        ...
        if (strCommand == NetMsgType::TXLOCKREQUEST) {
            ...
            CLMNode *pmn = mnodeman.Find(dstx.vin);
            if (pmn == NULL) {
                LogPrint("privatesend", "DSTX -- Can't find lmnode %s to verify %s\n",
                         dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return false;
            }

            if (!pmn->fAllowMixingTx) {
                LogPrint("privatesend", "DSTX -- LMNode %s is sending too many transactions %s\n",
                         dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return true;
            ...
            if (!dstx.CheckSignature(pmn->pubKeyLMNode)) {
...
//        LogPrintf("Main.cpp ProcessMessage() strCommand=%s\n", strCommand);
        ...
        if (found) {
            ...
            lmnodeSync.ProcessMessage(pfrom, strCommand, vRecv);
        } else {
            // Ignore unknown commands for extensibility
