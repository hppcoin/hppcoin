...
static int64_t nStartRewardTime = 1511365800;      
//  static int64_t nStartRewardTime = 1510834951;
/**
 * Global state
 */
...
bool CheckSpendHppcoinTransaction(const CTransaction &tx, CZerocoinEntry pubCoinTx, list <CZerocoinEntry> listPubCoin, libzerocoin::CoinDenomination targetDenomination, CValidationState &state, uint256 hashTx, bool isVerifyDB, int nHeight, bool isCheckWallet) {
    // Check vOut
...
//                              pubCoinItem.denomination, pubCoinItem.id, pubcoinId, pubCoinItem.nHeight);
                    if (pubCoinItem.denomination == targetDenomination &&
                        (pubCoinItem.id >= 0 && (uint32_t) pubCoinItem.id == pubcoinId) &&
                        pubCoinItem.nHeight != -1) {
                        LogPrint("CheckSpendHppcoinTransaction", "--## denomination = %d, id = %d, pubcoinId = %d height = %d\n",
  ...
  //  LogPrintf("transaction = %s\n", tx.ToString());
    ...
    if (tx.IsCoinBase()) {
        if (tx.vin[0].scriptSig.size() < 2 || tx.vin[0].scriptSig.size() > 100)
          {
            ...
            printf("tx.vin[0].scriptSig.size()=%d",tx.vin[0].scriptSig.size());
        ...
        if (!fTestNet && (GetAdjustedTime() > nStartRewardTime)) {
            ...
            FOUNDER_1_SCRIPT = GetScriptForDestination(CBitcoinAddress("HQ8NQPuoBveJULr4iQPR4SpYt2bXVhaAsf").Get());
            FOUNDER_2_SCRIPT = GetScriptForDestination(CBitcoinAddress("HX334rUg6faFtzN2KzbZmiyFpqu9kF3W9Y").Get());
            FOUNDER_3_SCRIPT = GetScriptForDestination(CBitcoinAddress("HTu8kJ8j3WjbdNdEsLzNcLW1g89rRnMbyP").Get());
            FOUNDER_4_SCRIPT = GetScriptForDestination(CBitcoinAddress("HYtVzbu3WKqFog2xEsRncdkCrzr4F6dAXe").Get());
            FOUNDER_5_SCRIPT = GetScriptForDestination(CBitcoinAddress("HQ8NQPuoBveJULr4iQPR4SpYt2bXVhaAsf").Get());
            FOUNDER_6_SCRIPT = GetScriptForDestination(CBitcoinAddress("HYtVzbu3WKqFog2xEsRncdkCrzr4F6dAXe").Get());
            FOUNDER_7_SCRIPT = GetScriptForDestination(CBitcoinAddress("HX334rUg6faFtzN2KzbZmiyFpqu9kF3W9Y").Get());
            FOUNDER_8_SCRIPT = GetScriptForDestination(CBitcoinAddress("HTu8kJ8j3WjbdNdEsLzNcLW1g89rRnMbyP").Get());
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            BOOST_FOREACH(const CTxOut &txout, tx.vout)
            {
                ...
                if (!isVerifyDB) {
                    ...
                    if (txout.nValue == libzerocoin::ZQ_LOVELACE * COIN) {
                        // Check vIn
                        if (!CheckSpendHppcoinTransaction(tx, pubCoinTx, listPubCoin, libzerocoin::ZQ_LOVELACE, state, hashTx,isVerifyDB, nHeight, isCheckWallet)) {
                            return state.DoS(100, error("CTransaction::CheckTransaction() : COIN SPEND TX IN ZQ_LOVELACE DID NOT VERIFY!"));
                        };
                    } else if (txout.nValue == libzerocoin::ZQ_GOLDWASSER * COIN) {
                        if (!CheckSpendHppcoinTransaction(tx, pubCoinTx, listPubCoin, libzerocoin::ZQ_GOLDWASSER, state, hashTx, isVerifyDB, nHeight, isCheckWallet)) {
                            return state.DoS(100, error("CTr[ansaction::CheckTransaction() : COIN SPEND TX IN ZQ_GOLDWASSER DID NOT VERIFY!"));
                        };
                    } else if (txout.nValue == libzerocoin::ZQ_RACKOFF * COIN) {
                        if (!CheckSpendHppcoinTransaction(tx, pubCoinTx, listPubCoin, libzerocoin::ZQ_RACKOFF, state, hashTx, isVerifyDB, nHeight, isCheckWallet)) {
                            return state.DoS(100, error("CTransaction::CheckTransaction() : COIN SPEND TX IN ZQ_RACKOFF DID NOT VERIFY!"));
                        };
                    } else if (txout.nValue == libzerocoin::ZQ_PEDERSEN * COIN) {
                        if (!CheckSpendHppcoinTransaction(tx, pubCoinTx, listPubCoin, libzerocoin::ZQ_PEDERSEN, state, hashTx, isVerifyDB, nHeight, isCheckWallet)) {
                            return state.DoS(100, error("CTransaction::CheckTransaction() : COIN SPEND TX IN ZQ_PEDERSEN DID NOT VERIFY!"));
                        };
                    } else if (txout.nValue == libzerocoin::ZQ_WILLIAMSON * COIN) {
                        if (!CheckSpendHppcoinTransaction(tx, pubCoinTx, listPubCoin, libzerocoin::ZQ_WILLIAMSON, state, hashTx, isVerifyDB, nHeight, isCheckWallet)) {
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params &consensusParams, int nTime) {
    bool fTestNet = (Params().NetworkIDString() == CBaseChainParams::TESTNET);
    // Just want to make sure no one gets a dime before 07 Nov 2017 
    if (nTime < nStartRewardTime && !fTestNet)
    {     LogPrintf(" GetBlockSubsidy nTime=%d nStartRewardTime=%u \n",nTime,nStartRewardTime);
        return 0;
     }
    ...
    // Subsidy is cut in half every 262 500 blocks which will occur approximately every 2 years.
    nSubsidy >>= halvings;
    return nSubsidy;
}
 ...
 * corresponding to pindexNew, to bypass loading it again from disk.
 */
bool static ReArrangeHppcoinMint(CValidationState &state, const CChainParams &chainparams, CBlockIndex *pindexNew, const CBlock *pblock) {
