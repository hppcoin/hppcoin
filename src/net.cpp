...
CNode *ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, bool fConnectToLMNode) {
    if (pszDest == NULL) {
        // we clean lmnode connections in CLMNodeMan::ProcessLMNodeConnections()
        // so should be safe to skip this and connect to local Hot MN on CActiveLMNode::ManageState()
        if (IsLocal(addrConnect) && !fConnectToLMNode)
            return NULL;
        LOCK(cs_vNodes);
        // Look for an existing connection
        CNode *pnode = FindNode((CService) addrConnect);
        if (pnode) {
            // we have existing connection to this node but it was not a connection to lmnode,
            // change flag and add reference so that we can correctly clear it later
            if (fConnectToLMNode && !pnode->fLMNode) {
                pnode->fLMNode = true;
            }
            pnode->AddRef();
            return pnode;
